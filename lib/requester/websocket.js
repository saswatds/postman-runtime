var PostmanWS = require('ws').Postman,
    ...
    WEBSOCKET_UPGRADE_EVENT_BASE = 'socket.upgrade.',
    WEBSOCKET_MESSAGE_EVENT_BASE = 'socket.event.',
    ...
    connect: function (id, request, protocolProfileBehavior, callback) {
        ...
        var self = this,
            ...
            onUpgrade = function (res, socket, head) {
                ...
                // By this point the `responseStart` callback has already been called
                // So we will have to emit an exception when upgrade needs to be blocked
                var responseHeaders = _.transform(res.headers, transformMultiValueHeaders, []),
                ...
                self.ws.on('message', function (message) {
                    self.emit(WEBSOCKET_MESSAGE_EVENT_BASE + id, null, message);
                });

                // Reference the sender and send for consumption
                self.emit(
                    WEBSOCKET_UPGRADE_EVENT_BASE + id,
                    null,
                    sdkResponse,
                    self.ws.send.bind(self.ws),
                    self.ws.close.bind(self.ws)
                );
            },
            onClose = function () {
                // When websocket is closed naturally then the request callback will not be called
                // because its attached to the response event instead the socket is closed and everything
                // is forgotten. All the parameter of req, res, cookie, history is ignored
                // @todo maybe borrow the req, res, cookie, history information from upgrade
                callback(self.error);
            };
        ...
        self.ws.on('close', onClose);

        // Request function here returns an aborter function that needs to be passed
        // back so that we can handle abort
        // NOTE: In the case where the connection is closed the callback is never fired,
        // We will have to manufacture a callback by listening on the close event on ws
        // instead
        // @todo intercept the abort to terminate the websocket connection as well
