    ...
    process: {
        ...
        websocketconn: function (payload, next) {
            ...
            async.applyEachSeries(prehelpers, context, self, function (err) {
                ...
                afterUpgrade = function (err, response, send, close) {
                    self.triggers.upgrade(err, context.coords, response, send, close);
                };
                ...
                async.auto({
                    ...
                    ref.websocket.on(WEBSOCKET_UPGRADE_EVENT_BASE + connectionId,
                        function (err, response, sender, closer) {
                            afterUpgrade(err, response, sender, closer);
                        });

                    ref.websocket.on(WEBSOCKET_MESSAGE_EVENT_BASE + connectionId, function (err, message) {
