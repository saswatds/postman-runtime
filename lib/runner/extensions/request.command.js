...
module.exports = {
    ...
    process: {
        request: function (payload, next) {
            ...
            // If this is an websocket protocol then we trigger a websocket connection
            const command = (context.item && context.item.request && context.item.request.protocol.is('websocket')) ?
                'websocketconn' : 'httprequest';

            // we do not queue instruction here, queueing will unblock the item command
            // to prepare for the next `event` instruction
            // at this moment request is not fulfilled, and we want to block it
            this.immediate(command, payload)
